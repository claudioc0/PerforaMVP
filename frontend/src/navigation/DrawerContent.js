...
const MENU_ITEMS = [
  { label: 'Treinos', route: 'WorkoutHistory', icon: 'barbell' },
  { label: 'Gráficos', route: 'Insights', icon: 'stats-chart' },
  { label: 'Progresso', route: 'Progress', icon: 'trending-up' },
      ...
      <Animated.View style={[styles.panel, { transform: [{ translateX }] }]}>
        ...
        {MENU_ITEMS.map((item) => (
          <TouchableOpacity key={item.route} style={styles.item} onPress={() => goTo(item.route)}>
            <Ionicons name={item.icon} size={22} color="#00FF66" style={styles.itemIcon} />
        ...
        <TouchableOpacity style={styles.item} onPress={handleLogout}>
          <Ionicons name="log-out" size={22} color="#888" style={styles.itemIcon} />
...
const styles = StyleSheet.create({
  ...
  title: { color: '#FFF', fontSize: 20, fontFamily: 'Orbitron_900Black', letterSpacing: 2, marginLeft: 10 },
